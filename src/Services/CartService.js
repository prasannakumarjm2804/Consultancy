  ...
  addToCart: async (product, quantity = 1) => {
    try {
      ...
      if (!isOnline() || !localStorage.getItem('token')) {
        ...
        // Ensure product has an id property that matches MongoDB _id
        const productWithId = {
          ...product,
          id: product.id || product._id // Use existing id or MongoDB's _id
        };
        
        const existingItemIndex = currentCart.findIndex(item => 
          item.id === productWithId.id || item._id === productWithId.id || item.productId === productWithId.id
        );
